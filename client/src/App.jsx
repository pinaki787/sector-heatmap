...
export default function App() {
  ...
  return <main className="terminal"><aside className="sidebar"><div className="brand"><span>◈</span><div><b>Sector Pulse</b><small>Trading workspace</small></div></div><nav aria-label="Dashboard views"><button type="button" aria-pressed={tab === 'sectors'} className={tab === 'sectors' ? 'active' : ''} onClick={() => setTab('sectors')}>Sector-led analysis</button><button type="button" aria-pressed={tab === 'broker'} className={tab === 'broker' ? 'active' : ''} onClick={() => setTab('broker')}>Live feed & positions <em>{open.length}</em></button></nav><div className="broker"><small>BROKER CONNECTION</small><strong role="status"><i aria-hidden="true" className={account.connected ? 'online' : ''}></i>{account.connected ? 'Fyers connected' : 'Fyers unavailable'}</strong><button type="button" onClick={() => openFyersAuthentication(setAuthStatus)}>Refresh authentication</button>{authStatus && <small role="status" aria-live="polite">{authStatus}</small>}</div></aside><section className="workspace">{tab === 'sectors' ? <><header><div><p className="eyebrow">SECTOR-LED ANALYSIS</p><h1>Leadership map</h1></div><div className="market-status" role="status"><i aria-hidden="true" className={heatmap.connected ? 'online' : ''}></i>{heatmap.connected ? 'Live Fyers feed' : 'Feed unavailable'}</div></header><section className="heatmap-panel"><div className="panel-title"><div><p className="eyebrow">SECTOR STRENGTH</p><h2>Choose a sector to inspect its stocks</h2></div><span>Refreshes every 30 minutes</span></div>{heatmapState.loading ? <div className="empty" role="status">Loading sectors…</div> : heatmapState.error ? <div className="empty error" role="alert">{heatmapState.error}</div> : sectors.length ? <div className="sector-grid" aria-label="Sector heat map">{sectors.map(sector => <button type="button" className={`sector ${sector.change == null ? 'unavailable' : sector.change >= 0 ? 'gain' : 'loss'} ${selectedName === sector.name ? 'selected' : ''}`} aria-pressed={selectedName === sector.name} onClick={() => setSelectedName(sector.name)} key={sector.name}><b>{sector.name}</b><small>{sector.index}</small><strong>{sector.change == null ? '—' : pct(sector.change)}</strong><span>{sector.weight}% weight · View stocks</span></button>)}</div> : <div className="empty">No sectors are available yet.</div>}</section><SectorDetails sector={selected} /></> : <><header><div><p className="eyebrow">LIVE FEED & POSITIONS</p><h1>Broker dashboard</h1></div><div className="market-status">Updates every 10 seconds</div></header><section className="metrics"><Metric label="Live P&L" value={money(account.pnl)} green={Number(account.pnl) >= 0} negative={Number(account.pnl) < 0}/><Metric label="Open positions" value={open.length}/><Metric label="Available funds" value={money(account.available_funds)}/><Metric label="Closed P&L" value={money(report.summary?.net_pnl)} green={Number(report.summary?.net_pnl) >= 0} negative={Number(report.summary?.net_pnl) < 0}/></section><section className="content"><section className="heatmap-panel"><div className="panel-title"><h2>Open positions</h2></div><div className="table">{open.length ? open.map((position, index) => <div className="row" key={index}><b>{position.symbol || position.symbol_name}</b><span>Qty {position.netQty ?? position.net_qty}</span><span>Avg {money(position.netAvg ?? position.net_avg)}</span><strong className={(position.pl ?? 0) >= 0 ? 'positive' : 'negative'}>{money(position.pl)}</strong></div>) : <div className="empty">No open positions.</div>}</div></section><section className="ticket"><div className="panel-title"><h2>Closed positions</h2></div><div className="pnl-nav" aria-label="Realised P and L period">{periods.map(([id, label]) => <button type="button" aria-pressed={period === id} key={id} className={period === id ? 'active' : ''} onClick={() => setPeriod(id)}>{label}</button>)}</div><div className="pnl-table">{report.records?.length ? report.records.map((record, index) => <div className="pnl-row" key={index}><b>{record.symbol}</b><span>{record.buy_qty} / {record.sell_qty}</span><strong className={record.pnl >= 0 ? 'positive' : 'negative'}>{money(record.pnl)}</strong></div>) : <div className="empty">No closed positions for this period.</div>}</div></section></section></>}</section></main>
}
